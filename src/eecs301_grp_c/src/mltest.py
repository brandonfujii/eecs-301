...
def classifyObject(k, training_dataset, test_row):
    distances = getKNeighbors(k,training_dataset, test_row)
    ...
    if neighbors[guessed_class] == 1:
 		guessed_class = min(distances, key=lambda x: x[1])[0]
...
def classifyObjects(training_dataset,test_dataset):
	...
	for i in range(0,len(test_dataset)):
	    attempts = {}
        for x in range(0, 5):
            guessed_class = classifyObject(k, training_dataset, test_dataset[i][1:])
            if guessed_class in attempts:
                attempts[guessed_class] = attempts[guessed_class] + 1
            else:
                attempts[guessed_class] = 1
        
        print attempts
        max_class = max(attempts.iteritems(), key=operator.itemgetter(1))[0]
        print "I'm guessing that's a" , max_class
	 	# if guessed_class == correct_class:
	 		# num_correct[correct_class] +=1
	# print "numcorrect", num_correct
	# for c in num_correct:
		# print (float(num_correct[c]) / num_test_points) * 100 , "percent correct at guessing object of type", c

def evaluateClassification(training_dataset, test_dataset, trials, correct_class):
    k = 2
    num_test_points = len(test_dataset)
    
    attempts = {"Triangle" : 0, "Cube" : 0, "Cylinder": 0}
    for x in range(0, trials):
        guessed_class = classifyObject(k, training_dataset, test_dataset[x][1:])
        if guessed_class in attempts:
            attempts[guessed_class] = attempts[guessed_class] + 1
        else:
            attempts[guessed_class] = 1
    ...
    print attempts
    # max_class = max(attempts.iteritems(), key=operator.itemgetter(1))[0]
    print float(attempts[correct_class]) / trials * 100, "percent correct at guessing object of type", correct_class
...
# test_dataset = loadDataset('tests/test-1480453231.87.csv')
